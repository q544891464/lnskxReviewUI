<template>
  <div class="disciplineManagement-container">
    <el-row>
      <el-col>
        <vab-query-form>
          <vab-query-form-left-panel :span="4">
            ...
            <el-button
              v-if="$perms('system_discipline_insert')"
              icon="el-icon-plus"
              type="primary"
              @click="handleInsert"
            > 添加 </el-button>
        ...
        >
          ...
          <el-table-column
            ...
            label="操作"
            width="130"
            v-if="$perms('system_discipline_insert') || $perms('system_discipline_update') || $perms('system_discipline_delete')"
          >
            <template v-slot="scope">

              <el-button
                v-if="$perms('system_discipline_update')"
                type="text"
              ...
              <el-dropdown trigger="click">
                ...
                >
                  ...
                  <el-dropdown-item
                    v-if="$perms('system_discipline_delete')"
...
<script>
  import { getTree, doDelete } from "@/api/system/discipline/disciplineManagement";
  import Edit from "./components/DisciplineManagementEdit";
    ...
    methods: {
      ...
      handleInsertByParent(row) {
        if (row.id) {
          ...
          // 上级名称
          rowTmp.parentName = row.disciplineName;
          // 上级编号
          rowTmp.parentCode = row.disciplineCode;
