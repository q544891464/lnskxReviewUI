...
<!-- 
      ...
      <el-table-column
        ...
        label="操作"
        width="200"
        v-if="$perms('system_expertmanagement_update') || $perms('system_expertmanagement_delete')"
      >
        <template v-slot="scope">
          <el-button
            v-if="$perms('system_expertmanagement_update')"
            type="text"
            @click="handleSetSubjectGroup(scope.row)"
          > 设置学科组 </el-button>
    ...
    <group ref="group" @fetchData="fetchData" ></group>
...
<script>
  ...
  export default {
    name: "SkxExpertManagement",
    components: { Edit, Import, Group },
    data() {
      return {
        list: null,
        listLoading: true,
    ...
    methods: {
      ...
      handleSetSubjectGroup(row) {
        if(row.id){
          console.log("单条");
          this.$refs["group"].show(row);
        }else{
      ...
      async fetchData() {
        ...
        const { data } = await getListRef(this.queryForm);
        if(isNotNull(data)){
          this.list = data.rows;
