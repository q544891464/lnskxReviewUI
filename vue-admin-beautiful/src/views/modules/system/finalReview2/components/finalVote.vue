<template>
  ...
  import { doUpdate } from "@/api/system/zhongping/SkxGetScoreFinalManagementApi";
  ...
  export default {
    ...
    methods: {
      show(row, type) {
        ...
        this.formData.setPrize = parseInt(row.finalPrePrize);
        ...
        this.$refs["elForm"].validate(async (valid) => {
          ...
          if (this.type == "pre") {
            const { msg } = await doUpdate({
              applyId: this.id,
              finalPrePrize: this.formData.setPrize,
            });
