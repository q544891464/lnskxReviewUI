...
<!--      <el-form-item label="成果原创性、前沿性、突破性创新内容和学术贡献" prop="innovation">
      ...
      <el-form-item label="职称级别" prop="firstAuthorTitleLevel">
         <el-select v-model="form.firstAuthorTitleLevel" placeholder="请选择职称级别" clearable
           :style="{width: '50%'}" v-bind:disabled="disabled">
           <el-option v-for="(item, index) in firstAuthorTitleLevelOptions" :key="index" :label="item.label"
             :value="item.value" :disabled="item.disabled"></el-option>
         </el-select>
       </el-form-item>
<!--      <el-form-item label="行政级别" prop="firstAuthorAdminLevel">
      ...
      <el-form-item label-width="150px" label="第一作者简介" prop="firstAuthorIntro">
        <el-input v-model="form.firstAuthorIntro" type="textarea"
      ...
      <el-form-item label="论文pdf上传" prop="uploadFile">
        ...
        :on-error="onError">
          <el-button slot="trigger" type="danger" icon="el-icon-upload2" v-bind:disabled="disabled">选取文件</el-button>
      ...
      <el-divider >补充材料</el-divider>

      <el-form-item label="论文发表刊物名称" prop="publicationName">
        <el-input v-model="form.publicationName" placeholder="请输入论文发表刊物名称" clearable :style="{width: '100%'}">
        </el-input>
      </el-form-item>

      <el-form-item label="其他证明材料" prop="uploadFileOther">
        <el-upload
        ref="fileimport2"
        accept=".pdf"
        ...
        :on-error="onError">
          <el-button slot="trigger" type="danger" icon="el-icon-upload2" v-bind:disabled="disabled">选取文件</el-button>
          <el-button type="danger" @click="viewFileOther" icon="el-icon-view">预览</el-button>
          <div slot="tip" class="el-upload__tip">如检索收录情况、被引用次数、会议交流的证明材料、有关部门提供的鉴定意见和取得效益的证明等能证明成果价值的相关证明材料。</div>
      ...
      <el-form-item label="申报表材料" prop="uploadFileCompleted">
        ...
        :on-error="onError">
          <el-button slot="trigger" type="danger" icon="el-icon-upload2" v-bind:disabled="disabled">选取文件</el-button>
          <el-button type="danger" @click="viewFileCompleted" icon="el-icon-view">预览</el-button>
          <div slot="tip" class="el-upload__tip">先提交本页，在「申报资料列表」中导出并下载申报表后，再回到这里将已经签字盖章的申报表上传，支持PDF格式。</div>
      ...
      <el-form-item size="large">
        <el-button type="primary" @click="submitForm('form')" v-bind:disabled="disabled">提交</el-button>
        <el-button @click="resetForm('form')" v-bind:disabled="disabled">重置</el-button>
      </el-form-item>
...
<script>
  import { doUpload,doInsert, doUpdate } from "@/api/system/apply/SysApplyManagementApi";
      ...
      return {
        ...
        rules: {
          ...
          firstAuthorId: [
            ...
            {required: true,trigger: 'blur',validator:validatorRule.IS_CITIZENID},
          ],
          firstAuthorRegion: [{
            required: true,
            message: '请输入地域',
          ...
          firstAuthorPost: [{required: true,message: '请填写职务',trigger: 'blur'}],
          firstAuthorTitle: [{required: true,message: '请填写职称',trigger: 'blur'}],
          firstAuthorAdminLevel: [{
            required: true,
            message: '请选择行政级别',
            trigger: 'change'
        ...
        relatedAchievementsOptions: [{
          "label": "有相关专利，或有3篇以上中文相关论文或1篇外文相关论文，或市级政府采用证明",
          "value": "A"
        }, {
          "label": "有2篇中文相关论文",
          "value": "B"
...
//    预览文件
      viewFileOther(){
            if(this.addFormOther.fileData == null && this.form.otherFilePath == null){
              this.$message.warning('请先上传PDF文件');
              return false;
            }
...
//    预览文件
      viewFileCompleted(){
            if(this.addFormCompleted.fileData == null && this.form.completedFilePath == null){
              this.$message.warning('请先上传PDF文件');
              return false;
            }
