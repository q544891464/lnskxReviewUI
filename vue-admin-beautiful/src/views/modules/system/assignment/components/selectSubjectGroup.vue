    ...
    >
      ...
      >
        <el-form-item label="设置学科组" prop="setSubjectGroup">
          <el-select
            v-model="formData.setSubjectGroup"
            placeholder="请选择学科组"
            @change="$forceUpdate()"
            clearable
            :style="{ width: '100%' }"
          >
            <el-option
              v-for="(item, index) in setSubjectGroupOptions"
              :key="index"
...
export default {
  ...
  methods: {
    show(row) {
      ...
      if (row.id) {
        this.id = row.id;
        this.formData.setSubjectGroup = row.subjectGroup;
        // console.log(row,"this.formData.setSubjectGroup");
      } else {
        this.ids = row;
        console.log("多条数据对话框");
      }
