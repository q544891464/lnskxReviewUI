<template>
  ...
  import { doUpdate,setBalance } from "@/api/system/zhongping/SkxGetScoreFinalManagementApi";
  ...
  export default {
    ...
    methods: {
      show(row, type) {
        ...
        this.formData.setPrize = parseInt(row.finalBalance);
        if(isNaN(this.formData.setPrize)){
      ...
      handelConfirm() {
        ...
        this.$refs["elForm"].validate(async (valid) => {
          ...
          if (this.type == "balance") {
            const { msg } = await setBalance({
              applyId: this.id,
              finalBalance: this.formData.setPrize,
            });
